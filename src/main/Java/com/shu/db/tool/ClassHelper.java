...
//                System.out.println("public boolean IsConflict :i = "+i +",j = "+j);
                Table[i][j] = false;
            }
        }
        for(ClassTime c:arrayList){
            for(DayTime d:c.getPeriod()){
                ...
                int end = d.getEnd();
                System.out.println("public boolean IsConflict: d = "+day +" start = "+start+" end = "+end);
...
//        PrintArr2(Table);
        for(DayTime d: classTime.getPeriod()){
            ...
            for(int j = start;j<=end;j++){
                if(Table[j][day] == true){
                    isConflict = true;
                    System.out.println("Class Helper is conflict");
                    break;
                }
            }
        }
